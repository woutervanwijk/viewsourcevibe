  ...
  group('File Type Detection Tests', () {
    ...
    group('Extension-Based Detection', () {
      ...
      test('Should detect JavaScript files by extension', () async {
        expect(
            await detector.detectFileType(filename: 'script.js'), 'JavaScript');
        expect(await detector.detectFileType(filename: 'script.mjs'),
            'JavaScript');
      });

      test('Should detect TypeScript files by extension', () async {
        expect(
            await detector.detectFileType(filename: 'script.ts'), 'TypeScript');
      });
    ...
    group('Content-Based Detection', () {
      ...
      test('Should detect Markdown content without extension', () async {
        const markdownContent =
            '# Heading\n## Subheading\n**bold text**\n* item\n1. numbered';
        expect(await detector.detectFileType(content: markdownContent),
            'Markdown');
      });
    ...
    group('Priority Detection', () {
      test('Should prioritize extension over content when both are present',
          () async {
        // Python-like content but with .js extension
        const content = 'def hello():\n    print("Hello")';
        expect(
            await detector.detectFileType(
                filename: 'test.js', content: content),
            'JavaScript');
      });
      ...
      test('Should use content detection when extension is unknown', () async {
        ...
        expect(
            await detector.detectFileType(
                filename: 'test.xyz', content: pythonContent),
            'Python');
      });
    });
    ...
    group('Integration with HtmlService', () {
      ...
      test('Should generate appropriate filenames for JavaScript content',
          () async {
      ...
      test('Should handle edge cases gracefully', () async {
        expect(await htmlService.detectFileTypeAndGenerateFilename('', ''),
            'index.html');
        expect(await htmlService.detectFileTypeAndGenerateFilename('index', ''),
            'index.html');
        expect(await htmlService.detectFileTypeAndGenerateFilename('/', ''),
            'index.html');
      });
    ...
    group('Fallback Detection', () {
      test('Should handle edge cases gracefully', () async {
        ...
        expect(
            await detector.detectFileType(
                content: 'random text without specific patterns'),
            'Text');
      });
    });
    ...
    group('Binary File Detection', () {
      ...
      test('Should detect files with null bytes as binary', () async {
        ...
        final binaryBytes = Uint8List.fromList(
            [0x48, 0x65, 0x6C, 0x6C, 0x00, 0x57, 0x6F, 0x72, 0x6C, 0x64]);
      ...
      test('Should allow text files with binary extensions if content is text',
          () async {
      ...
      test('Should provide meaningful error messages for binary files',
          () async {
