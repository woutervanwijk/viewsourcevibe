...
void main() {
  group('iOS Share Extension Tests', () {
    test('Should handle iOS share extension URL schemes correctly', () async {
      ...
      for (final testCase in testCases) {
        final uriString =
            '${testCase['scheme']}://${testCase['host']}?${testCase['query']}';
    ...
    test('Should handle iOS file paths correctly', () async {
      ...
      for (final filePath in iosFilePaths) {
        ...
        expect(fileName.contains('/'),
            false); // Should not contain path separators
      }
    });
    ...
    test('Should handle various iOS content types', () async {
      ...
      final contentTypes = [
        {'type': 'url', 'content': 'https://example.com', 'isUrl': true},
        {
          'type': 'file',
      ...
      for (final contentType in contentTypes) {
        if (contentType['type'] == 'file') {
          expect(SharingService.isFilePath(contentType['filePath'] as String),
              true);
    ...
    test('Should handle iOS share extension complex scenarios', () async {
      ...
      final complexFilePath =
          '/Users/test/My Documents/Project Files/source code.html';
      expect(SharingService.isFilePath(complexFilePath), true);

      // Test 3: File name extraction from complex paths
      final fileName =
          SharedContentManager.extractFileNameFromPath(complexFilePath);
